...
const UserDisplayPage = () => {
  ...
  return <>
      <TextField style={{
      marginBottom: '1.5rem'
    }} variant='outlined' value={input} onChange={handleInputChange} fullWidth placeholder='Search users...' />
      <AnimatePresence>
        <Grid spacing={3} container>
          {users.filter(user => `${user.firstName} ${user.lastName}`.toLowerCase().includes(search)).map(user => <Grid key={user.id} xs={12} sm={6} md={4} lg={3} item>
                <Card>
                  <motion.div className={classes.userCard} key={user.id} animate={{
              opacity: 1,
              scale: 1
            }} transition={{
              duration: 0.3
              ...
              scale: 0.95
                    ...
                    <CardMedia className={classes.userCardItems} component='img' height='120' image={user.avatar} />
                ...
                justifyContent: 'flex-end'
                      ...
                      <Button style={{
                  marginRight: '.5rem'
                }} size='small' color='primary' onClick={() => handleRemove(user.id)}>
                        REMOVE USER
                      </Button>
                    </div>
                  </motion.div>
      ...
      marginTop: '1.5rem'
        ...
        <Link to='/add_user'>
          <Button startIcon={<PersonAddIcon fontSize='large' />} style={{
          float: 'right'
